...
- `pq.scans.runCloud(...)` / `run_cloud(...)` / `RunCloudAsync(...)` → live `POST /v1/scans/cloud`
- `pq.scans.runUrl(...)` / `run_url(...)` / `RunUrlAsync(...)` → live `POST /v1/scans/url`
...
credentials and raw inventory remain inside the customer environment.
